...
#define CW -1		// clockwise
...
#ifdef PEPSDLL
		void	ToPeps(int id);										// convert matrix to peps matrix
		void	FromPeps(int id);									// from a Peps matrix to matrix
#endif
	};
	...
	public:
		...
		void	FromPeps(int id);													// point from Peps
...
#define HORIZ_CLINE CLine(Point(0,0), 1, 0)
	...
	public:
		...
		Point3d p1;			// end
		Point3d pc;			// centre
		Vector3d normal;	// normal to arc
		int	  dir;			// arc direction (CW or ACW or 0 for straight)
		bool ok;

		bool	returnSpanProperties;	// set if properties below are set
		Vector3d vs;			// direction at start or for straight
		Vector3d ve;			// direction at span end
		...
		int Split(double tolerance);												// returns number of splits
		void SplitMatrix(int num_vectors, Matrix* matrix);							// returns incremental matrix from split
		void minmax(Box3d& box, bool start = true);									// minmax of span
		void minmax(Point3d& pmin, Point3d& pmax, bool start = true);				// minmax of span
//		int Intof(Span& sp, Point3d& pInt1, Point3d& pInt2);
		void Transform(const Matrix& m, bool setprops = true);
//		Point3d Near(Point3d& p, double& t);										// returns the near point to span from p 
//		Point3d Near(Point3d& p){double t; return Near(p, t);};						// returns the near point to span from p 
//		Point3d	Mid();																// midpoint of a span
//		bool OnSpan(Point3d& p);													//  tests if p is on sp *** FAST TEST p MUST LIE on unbounded span

		// constructor
		Span3d(Point3d& sp, Point3d& ep) {dir = LINEAR; p0 = sp; p1 = ep; SetProperties(true); /*ok = p0.ok;*/};
		Span3d(int direction, Vector3d n, Point3d& sp, Point3d& ep, Point3d& c) {dir = direction; normal = n; p0 = sp; p1 = ep; pc = c; SetProperties(true); /*ok = p0.ok;*/};
		Span3d() { normal = Z_VECTOR; ok = false;};
//		Span3d(int spandir, Point3d& pn, Point3d& pf, Point3d& c) { dir = spandir; p0 = pn, p1 = pf, pc = c; SetProperties(true);};
...
#define SPANSTORAGE 32			// lessens number of object pointers
	...
	public:
		...
		// tech
		std::string tech;				// tech string

		// misc
		std::string misc;				// misc string

		// taper
		std::string taper;				// taper string
		double eTaper;				// end taper angle  (encoded from taper)
		double radiusSec;			// secondary radius	
		int	corner;					// corner condition			ditto
		bool tangent;				// next span is tangent (for convenience not set by class library - see pro_pp -> OutputMain)
		bool external;				// set if next span is an external corner

		// iso blend
		bool Blend;					// by analysis for minimal NC output
		double isoRadius;			// radius
		Point Intersection;			// intersection of neighbour for radius

		Point nciPosition;			// nci position (only needed to prevent duplicates (eg.roll around offset corners)
...
#pragma warning(default:4522)
...
#ifdef PARASOLID
	static PK_BODY_t MakeSheetBody(Kurve &k0, Kurve &k1, int start_spanno, int n_spans);
	static PK_BODY_t MakeDegenerateSheetBody(Kurve &k0, Kurve &k1, int spanno);
	static void MakeSurfaceReversalSheetBody(Kurve &k0, Kurve &k1, int spanno, Point3d& p, PK_BODY_t parts[2]);
	static int is_surface_reversal(Span3d &s0, Span3d &s1, Point3d& p);
	PK_BODY_t MakeRuledSurface(PK_BODY_t *wires);
	int MakeMinimumBody(Point3d& p);
	int UniteSheets(PK_BODY_t* sheet, PK_BODY_t part_sheet);
	static int thicken_sheet(PK_BODY_t sheet, double thickness);
	static int TrimSheet(PK_BODY_t* sheet, PK_BODY_t part_sheet);
#endif
	...
	public:
		...
		// methods
		void	Add(char* figure, double offset, bool onspan);						// add nci's from a Peps figure
		int		Get(int spannumber, Span& Sp , WireExtraData& extra, bool returnSpanProperties);
		WireExtraData* Get(int spanVertexNumber);
		void	FindBlends(double offset);														// apply iso corners to extra data
		void	endExtra();
		void	Add(int spanVertexNumber, int nciType, char* string, Point& pNCI);
	private:
		void	Add(int spanVertexNumber, double radius, Point& inters);			// adds iso extra data
		void	AddExtra(char* figure, int spanvertex, Span& offsetRef);
		void	AddExtra(char* figure, int spanvertex, Point& p);
		bool	DuplicateExtra(int nciType, char* string, Point& nciPos);			// duplicate extradata ?
	...
	class GroupPoint{
	protected:
		double x[SPANSTORAGE], y[SPANSTORAGE], z[SPANSTORAGE];											// point
	public:
		// methods
		void	Add(int offset, double* p);
		void	Add(int offset, double x, double y, double z = 0);
		void	Get(int offset, double* p);
	};
	class Group : public Matrix{
	protected:
		vector<GroupPoint*> m_points;
		bool		m_started;
		int			m_nPoints;					// number of points in Group

	public:
		// constructors etc...
		Group();
		Group(Group& g);
		const Group& operator= (Group& g );

		// destructor
		~Group();

		// methods
		void	Start();											// start a new group
		void	Add(double* p);										// add a new point (to the end)
		void	Add(double x, double y, double z = 0);				// add a new point (to the end)
		int		Get(int spannumber, double& x, double& y, double& z, int transform = true);
		int		Get(int spannumber, double*p, int transform = true);
		//	int		Get(int spannumber, Point& p, int transform = true);
		void	Add(Point3d& p);
		void	Add(Point p) { Add(Point3d(p));};
		void	Add(Group* g);

		void	ToPeps(int id, bool draw = true);					// convert group to peps group
		void	FromPeps(int id);									// from a Peps to group
	};
#endif
