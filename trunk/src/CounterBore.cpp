...
std::list< CCounterBore::Point3d > CCounterBore::DrillBitVertices( const CCounterBore::Point3d & origin, const double radius, const double length ) const
{
	...
	{
		double theta = alpha * i;
		CCounterBore::Point3d topEdge( cos( theta ) * radius, sin( theta ) * radius, 0 );
		...
		CCounterBore::Point3d veryTip( origin );
		veryTip.x = origin.x;
		veryTip.y = origin.y;
