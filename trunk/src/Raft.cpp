		...
		if(span_object){
			...
			else
			{
				if (type == CircleType)
				{
					...
					points.push_back( std::make_pair(0, gp_Pnt( c[0] - small_amount, c[1] + radius, c[2] )) ); // north (almost)
					...
					points.push_back( std::make_pair(-1, gp_Pnt( c[0] - small_amount, c[1] + radius, c[2] )) ); // north (almost)
